/**
 * Migration script: Northside Toyota Feb 2026
 * Reads reference/Northside_Toyota_Feb_26.xlsx and inserts into Supabase.
 *
 * Usage: npx tsx scripts/migrate-northside-toyota.ts
 */
...
const FILE = path.resolve(__dirname, "../reference/Northside_Toyota_Feb_26.xlsx");
...
async function main() {
  ...
  // ── Step 1: Create Event ──────────────────────────────────────────────
  console.log("Step 1: Creating Northside Toyota event...");
  const { data: event, error: evErr } = await sb
    .from("events")
    .insert({
      dealer_name: "NORTHSIDE TOYOTA",
      franchise: "TOYOTA",
      city: "CHICAGO",
      state: "IL",
  ...
  // Rows 1–18, col B(1) = name
  // Index 6 HALEY DELUDE = team_leader
  // Index 12 MAYCON MIKE GUIMARAES = manager
  ...
  for (let i = 1; i <= 18; i++) {
    ...
    let type = "rep";
    if (i === 6) type = "team_leader"; // HALEY DELUDE
    else if (i === 12) type = "manager"; // MAYCON MIKE GUIMARAES
  ...
  // Row 2+ (index 1+)
  // A(0)=hat_num, C(2)=status, D(3)=stock_num, E(4)=year, F(5)=make,
  // G(6)=model, H(7)=class, I(8)=color, J(9)=odometer, K(10)=vin,
  // L(11)=trim, M(12)=age, N(13)=drivetrain, O(14)=kbb_trade,
  // P(15)=kbb_retail, Q(16)=cost
  ...
  // Data starts row 10 (index 9), deal# in col E (index 4)
  // E(4)=deal_num, G(6)=store, H(7)=stock_num, I(8)=customer_name,
  // J(9)=customer_zip, K(10)=new_used, L(11)=year, M(12)=make,
  // N(13)=model, O(14)=cost, P(15)=age, Q(16)=trade_year,
  // R(17)=trade_make, S(18)=trade_model, T(19)=trade_miles,
  // U(20)=acv, V(21)=payoff, W(22)=salesperson, X(23)=sp2,
  // Y(24)=front_gross, Z(25)=lender, AA(26)=rate, AB(27)=reserve,
  // AC(28)=warranty, AD(29)=aft1, AE(30)=gap, AF(31)=fi_totals, AG(32)=total_gross, AH(33)=notes
  const dealRows: Record<string, unknown>[] = [];
  const skippedSp = new Set<string>();

  // Fuzzy match helper: "RJ" → "RJ JOHNSON JR"
  function matchSp(raw: string | null): string | null {
    if (!raw) return null;
    if (nameToId[raw]) return nameToId[raw];
    // Fuzzy: check if any name starts with the raw value
    if (raw === "RJ" && nameToId["RJ JOHNSON JR"]) return nameToId["RJ JOHNSON JR"];
    return null;
  }
  ...
  // ── Summary ───────────────────────────────────────────────────────────
  console.log("\n══════════════════════════════════════════════");
  console.log("Done! Northside Toyota migration complete.");
