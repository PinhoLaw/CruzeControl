/**
 * Migration script: Michigan City Ford July 2025
 * Reads reference/Michigan_City_Ford_July_25.xlsx and inserts into Supabase.
 *
 * Usage: npx tsx scripts/migrate-michigan-city-july25.ts
 */
...
const FILE = path.resolve(__dirname, "../reference/Michigan_City_Ford_July_25.xlsx");
...
async function main() {
  ...
  // ── Step 1: Create Event ──────────────────────────────────────────────
  console.log("Step 1: Creating Michigan City Ford July 2025 event...");
  ...
  // Rows 1–21, col D(3) = name, col G(7) for role detection
  // Index 6 TREVON HALL = team_leader (col G = "Team Leader")
  // Index 12 JAMES LYNCH = manager (col G = "F&I Manager")
  // Skip HOUSE and blank rows
  const spRows: { name: string; type: string }[] = [];
  for (let i = 1; i <= 21; i++) {
    const r = roster[i] as unknown[];
    const name = safeStr(r?.[3]);
    if (!name) continue;
    if (name === "HOUSE") continue;

    const colG = safeStr(r?.[7]);
    let type = "rep";
    if (colG === "Team Leader") type = "team_leader";
    else if (colG === "F&I Manager") type = "manager";
  ...
  // Row 2+ (index 1+)
  // A(0)=hat_num, B(1)=location, C(2)=status, D(3)=stock_num,
  ...
  // AD(29)=warranty, AE(30)=aft1, AF(31)=gap, AM(38)=notes

  // Fuzzy match: trim trailing whitespace from salesperson names
  ...
  // Row 5+ (index 4+), where col A(0) is a number or col D(3) is a zip
  // Skip TOTAL rows. A(0)=pieces_sent, C(2)=town, D(3)=zip_code,
  // F(5)=day_1 through O(14)=day_10
  ...
  // ── Summary ───────────────────────────────────────────────────────────
  console.log("\n══════════════════════════════════════════════");
  console.log("Done! Michigan City Ford July 2025 migration complete.");
